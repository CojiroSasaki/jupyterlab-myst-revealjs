...
```bash
python -m py_compile jupyterlab_myst_revealjs/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'jupyterlab_myst_revealjs:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyterlab_myst_revealjs:open-panel';
  export const refreshData = 'jupyterlab_myst_revealjs:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'jupyterlab_myst_revealjs:plugin',
  autoStart: true,
...
- **Output**: Creates files in `lib/` and `jupyterlab_myst_revealjs/labextension/`
...
- Ask user to search for the extension ID (`jupyterlab_myst_revealjs`) to see if it loaded
...
ls -la jupyterlab_myst_revealjs/labextension/  # Should contain bundled extension
```
...
```css
.jp-jupyterlab-myst-revealjs-widget {
  padding: 8px;
}
```
...
- **Plugin ID** (in `src/index.ts`): `'jupyterlab_myst_revealjs:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyterlab_myst_revealjs:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyterlab-myst-revealjs-ClassName`
