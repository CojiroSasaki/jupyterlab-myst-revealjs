...
jest.mock('@jupyterlab/cells', () => {
    class MockCodeCell {
        ...
        initializeState() { /* noop */ }
        addClass(cls: string) {
            this._classes.push(cls);
            this.node.classList.add(cls);
        }
        hasClass(cls: string) { return this._classes.includes(cls); }
        dispose() { /* noop */ }
    }
...
describe('SlideBuilder', () => {
    describe('slide structure', () => {
        ...
        it('splits cells into separate slides at slide boundaries', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Slide 1'),
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Slide 2'),
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Slide 3')
            ];
            const dom = await buildSlides(cells);
        ...
        it('appends continuation cells to the current slide', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Title'),
                createCellModel('markdown', { slideshow: { slide_type: '-' } }, 'paragraph'),
        ...
        it('creates nested sections for subslides', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Main'),
                createCellModel('markdown', { slideshow: { slide_type: 'subslide' } }, '# Sub 1'),
                createCellModel('markdown', { slideshow: { slide_type: 'subslide' } }, '# Sub 2')
            ];
            const dom = await buildSlides(cells);
        ...
        it('wraps fragment cells with the fragment class', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Title'),
                createCellModel('markdown', { slideshow: { slide_type: 'fragment' } }, 'appear later')
            ];
            const dom = await buildSlides(cells);
        ...
        it('wraps notes cells in an aside element', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Title'),
                createCellModel('markdown', { slideshow: { slide_type: 'notes' } }, 'speaker notes')
            ];
            const dom = await buildSlides(cells);
        ...
        it('excludes skip cells from the DOM', async () => {
            const cells = [
                createCellModel('markdown', { slideshow: { slide_type: 'slide' } }, '# Title'),
                createCellModel('markdown', { slideshow: { slide_type: 'skip' } }, 'hidden'),
                createCellModel('markdown', { slideshow: { slide_type: '-' } }, 'visible')
            ];
            const dom = await buildSlides(cells);
    ...
    describe('tag processing', () => {
        ...
        it('applies hide-cell class to code cells and markdown cells', async () => {
            ...
            expect(inner.querySelector('.jp-Slideshow-markdownCell.jp-Slideshow-hideCell'
                + ', .jp-Slideshow-hideCell')).not.toBeNull();
            // Code cell
    ...
    describe('lifecycle', () => {
        ...
        it('findFocusedCodeCell returns the focused code cell', async () => {
            ...
            const container = dom.querySelector('.jp-Slideshow-codeCellContainer') as HTMLElement;
            container.focus();
